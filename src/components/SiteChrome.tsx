...
import { CDN_ROOT, SceneGate, SITE_CHROME_IMAGES } from "@/components/sultan-shared";
...
export function SiteFooter() {
  return (
    <footer className="relative z-20 w-full shrink-0 border-t-2 border-footer-border bg-footer-surface font-display text-footer-copy">
      <SceneGate images={SITE_CHROME_IMAGES}>
                ...
                >
                  {"image" in item && item.image ? (
                    <img
                      src={item.image}
                      alt={item.label}
                      className="h-7 w-auto object-contain [image-rendering:pixelated] sm:h-8"
                    />
                  ) : (
                    item.label
                  )}
